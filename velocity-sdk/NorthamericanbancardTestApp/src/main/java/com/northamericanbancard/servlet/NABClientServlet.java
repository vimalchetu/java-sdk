...
public class NABClientServlet extends HttpServlet {
	...
	public void service(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			VelocityProcessor velocityProcessor = new VelocityProcessor(
					sessionToken, identityToken, appProfileId,
					merchantProfileId, workFlowId, isTestAccount);
			LOG.debug("Session token >>>>>>>>>>>> "
					+ velocityProcessor.getSessionToken());
			...
			if (txnName != null
					&& txnName.equalsIgnoreCase(NABConstants.VERIFY)) {
				velocityResponse = velocityProcessor
						.verify(getVerifyRequestAuthorizeTransactionInstance(req));
				LOG.debug("velocityResponse >>>>>>>>>>>> " + velocityResponse);
				if (velocityResponse != null
						&& velocityResponse.getBankcardTransactionResponse() != null) {
					paymentAccountDataToken = velocityResponse
							.getBankcardTransactionResponse()
							.getPaymentAccountDataToken();
					session.setAttribute("paymentAccountDataToken",
							paymentAccountDataToken);
				}
				session.setAttribute("verifyRequestXML",
						velocityProcessor.getTxnRequestLog());
				session.setAttribute("verifyResponseXML",
						velocityProcessor.getTxnResponseLog());
				session.setAttribute("txnName", "Verify");
				LOG.debug("VelocityResponse >>>>>>>>>> " + velocityResponse);
			}
			/* Invokes the Authorize transaction from the VelocityTestApp */
			else if (txnName != null
					&& txnName.equalsIgnoreCase(NABConstants.AUTHORIZE)) {
				AuthorizeTransaction objAuthorizeTransaction = null;
				if (req.getParameter("securePaymentAccountData") == null
						|| req.getParameter("securePaymentAccountData").trim()
								.isEmpty()) {
				...
				velocityResponse = velocityProcessor
						.authorize(objAuthorizeTransaction);
				if (velocityResponse != null
						&& velocityResponse.getBankcardTransactionResponse() != null) {
					session.setAttribute("transactionId", velocityResponse
							.getBankcardTransactionResponse()
							.getTransactionId());
				}
				if (session.getAttribute("txnList") == null) {
					...
					if (velocityResponse != null
							&& velocityResponse
									.getBankcardTransactionResponse() != null) {
						txnList.add(velocityResponse
								.getBankcardTransactionResponse()
								.getTransactionId());
					}
					session.setAttribute("txnList", txnList);
				} else {
					List<String> txnList = (List<String>) session
							.getAttribute("txnList");
					if (velocityResponse != null
							&& velocityResponse
									.getBankcardTransactionResponse() != null) {
						txnList.add(velocityResponse
				...
				LOG.debug("Authorize VelocityResponse >>>>>>>>>> "
						+ velocityResponse);
			}
			/*
			 * Invokes the Authorize transaction without token from the
			 * VelocityTestApp
			 */
			else if (txnName != null
					&& txnName.equalsIgnoreCase(NABConstants.AUTHORIZEWOTOKEN)) {
				AuthorizeTransaction objAuthorizeTransaction = null;
				if (req.getParameter("securePaymentAccountData") == null
						|| req.getParameter("securePaymentAccountData").trim()
								.isEmpty()) {
				...
				objAuthorizeTransaction.getTransaction().getTenderData()
						.getPaymentAccountDataToken().setValue("");
				velocityResponse = velocityProcessor
						.authorize(objAuthorizeTransaction);
				...
				LOG.debug("AuthorizeWotoken VelocityResponse >>>>>>>>>> "
						+ velocityResponse);
				session.setAttribute("txnName", "AuthorizeWithoutToken");
			}
			/*
			 * Invokes the AuthorizeAndCapture transaction from the
			 * VelocityTestApp
			 */
			else if (txnName != null
					&& txnName
							.equalsIgnoreCase(NABConstants.AUTHORIZEANDCAPTURE)) {
				...
				velocityResponse = velocityProcessor
						.authorizeAndCapture(objAuthorizeAndCaptureTransaction);
				session.setAttribute("txnName", "AuthorizeAndCapture");
				LOG.debug("AuthAndCapture VelocityResponse >>>>>>>>>> "
						+ velocityResponse);
			}
			/*
			 * Invokes the AUthorizeAndCapture transaction without token from
			 * the VelocityTestApp
			 */
			else if (txnName != null
					&& txnName
							.equalsIgnoreCase(NABConstants.AUTHORIZEANDCAPTUREWOTOKEN)) {
				AuthorizeAndCaptureTransaction objAuthorizeAndCaptureTransaction = null;
				if (req.getParameter("securePaymentAccountData") == null
						|| req.getParameter("securePaymentAccountData").trim()
								.isEmpty()) {
				...
				objAuthorizeAndCaptureTransaction.getTransaction()
						.getTenderData().getPaymentAccountDataToken()
						.setValue("");
				velocityResponse = velocityProcessor
						.authorizeAndCapture(objAuthorizeAndCaptureTransaction);
				session.setAttribute("txnName",
						"AuthorizeAndCaptureWithoutToken");
				LOG.debug("AuthAndCaptureWoToken VelocityResponse >>>>>>>>>> "
						+ velocityResponse);
			}
			/* Invokes the Capture transaction from the VelocityTestApp */
			else if (txnName != null
					&& txnName.equalsIgnoreCase(NABConstants.CAPTURE)) {
				AuthorizeTransaction objAuthorizeTransaction = getAuthorizeRequestAuthorizeTransactionInstance(req);
				VelocityResponse objVelocityResponse = velocityProcessor
						.authorize(objAuthorizeTransaction);
				ChangeTransaction objChangeTransaction = getCaptureTransactionInstance(req);
				objChangeTransaction.getDifferenceData().setTransactionId(
						objVelocityResponse.getBankcardTransactionResponse()
								.getTransactionId());
				velocityResponse = velocityProcessor
						.capture(objChangeTransaction);
				session.setAttribute("txnName", "Capture");
				LOG.debug("capture VelocityResponse >>>>>>>>>> "
						+ velocityResponse);
			}
			/* Invokes the Undo transaction from the VelocityTestApp */
			else if (txnName != null
					&& txnName.equalsIgnoreCase(NABConstants.UNDO)) {
				AuthorizeTransaction objAuthorizeTransaction = getAuthorizeRequestAuthorizeTransactionInstance(req);
				VelocityResponse objVelocityResponse = velocityProcessor
						.authorize(objAuthorizeTransaction);
				Undo objUndo = getUndoTransactionInstance(req);
				objUndo.setTransactionId(objVelocityResponse
						.getBankcardTransactionResponse().getTransactionId());
				velocityResponse = velocityProcessor.undo(objUndo);
				LOG.debug("undo VelocityResponse >>>>>>>>>> "
						+ velocityResponse);
				session.setAttribute("txnName", "Undo");
			}
			/* Invokes the Adjust transaction from the VelocityTestApp */
			else if (txnName != null
					&& txnName.equalsIgnoreCase(NABConstants.ADJUST)) {
				AuthorizeTransaction objAuthorizeTransaction = getAuthorizeRequestAuthorizeTransactionInstance(req);
				VelocityResponse objVelocityResponse = velocityProcessor
						.authorize(objAuthorizeTransaction);
				...
				if (objVelocityResponse.getBankcardTransactionResponse() != null) {
					...
					objVelocityResponse = velocityProcessor
							.capture(objChangeTransaction);
					if (objVelocityResponse.getBankcardCaptureResponse() != null) {
						Adjust objAdjust = getAdjustTransactionInstance(req);
						objAdjust.getDifferenceData().setTransactionId(
								objVelocityResponse
										.getBankcardCaptureResponse()
										.getTransactionId());
			...
			/* Invokes the ReturnById transaction from the VelocityTestApp */
				...
				if (objVelocityResponse.getBankcardTransactionResponse() != null) {
					...
					if (objVelocityResponse.getBankcardCaptureResponse() != null) {
						...
						objReturnById.getDifferenceData().setTransactionId(
								objVelocityResponse
										.getBankcardCaptureResponse()
										.getTransactionId());
						velocityResponse = velocityProcessor
								.returnById(objReturnById);
					}
				}

				LOG.debug("invokeReturnByIdRequest VelocityResponse >>>>>>>>>> "
						+ velocityResponse);
				session.setAttribute("txnName", "ReturnById");
			}
			/* Invokes the ReturnUnLinked transaction from the VelocityTestApp */
			else if (txnName != null
					&& txnName.equalsIgnoreCase(NABConstants.RETURNUNLINKED)) {
				...
				velocityResponse = velocityProcessor
						.returnUnlinked(objreturnUnlinkedTransaction);
				if (velocityResponse.getBankcardTransactionResponse() != null) {
					session.setAttribute("transactionId", velocityResponse
							.getBankcardTransactionResponse()
							.getTransactionId());
				...
				LOG.debug("ReturnUnlinkedRequest VelocityResponse >>>>>>>>>> "
						+ velocityResponse);
			}
			/*
			 * Invokes the ReturnUnlinked transaction without token from the
			 * VelocityTestApp
			 */
			else if (txnName != null
					&& txnName
							.equalsIgnoreCase(NABConstants.RETURNUNLINKEDWOTOKEN)) {
				ReturnTransaction objreturnUnlinkedTransaction = null;
				if (req.getParameter("securePaymentAccountData") == null
						|| req.getParameter("securePaymentAccountData").trim()
								.isEmpty()) {
				...
				objreturnUnlinkedTransaction.getTransaction().getTenderData()
						.getPaymentAccountDataToken().setValue("");
				velocityResponse = velocityProcessor
						.returnUnlinked(objreturnUnlinkedTransaction);
				if (velocityResponse.getBankcardTransactionResponse() != null) {
				...
				LOG.debug("ReturnUnlinkedWoToken VelocityResponse >>>>>>>>>> "
						+ velocityResponse);
			}
			/* Invokes the QueryTransactionsDetail from the VelocityTestApp */
			else if (txnName != null
					&& txnName
							.equalsIgnoreCase(NABConstants.QUERYTRANSACTIONDETAIL)) {
				...
				if (qtdTxns != null && qtdTxns.length > 0) {
					LOG.debug("InvokeQueryTransactionDetailRequest Query Transactionssssss >>>>>> "
							+ qtdTxns);
					for (String qtd : qtdTxns) {
						objQueryTransactionsDetail
								.getQueryTransactionsParameters()
								.getTransactionIds().add(qtd.trim());
					}
				}
				velocityResponse = velocityProcessor
						.queryTransactionsDetail(objQueryTransactionsDetail);
				if (velocityResponse.getTransactionDetailList() != null) {
					LOG.debug("InvokeQueryTransactionDetailRequest TransactionDetailList size >>>>>> "
							+ velocityResponse.getTransactionDetailList()
									.size());
					req.setAttribute("transactionDetailList",
							velocityResponse.getTransactionDetailList());
			...
			/* Invokes the CaptureAll from the VelocityTestApp */
					...
					&& txnName.equalsIgnoreCase(NABConstants.CAPTURE_ALL)) {
				...
				if (velocityResponse.getArrayOfResponse() != null) {
					session.setAttribute("transactionId", velocityResponse
							.getArrayOfResponse().getResponse()
							.getTransactionId());
				}
				session.setAttribute("txnName", "captureAll");
				LOG.debug("captureAll VelocityResponse >>>>>>>>>> "
						+ velocityResponse);
			}
			...
			if (e.getMessage() != null) {
				req.setAttribute("ErrorMessage", (e.getMessage().substring(e
						.getMessage().indexOf("<"))).trim());
			}
		}
	...
	/**
	 * Generating the Verify input AuthorizeTransaction instance from the User
	 * inputs.
	 * 
	 * @author Vimal Kumar
	...
	AuthorizeTransaction getVerifyRequestAuthorizeTransactionInstance(
		...
		authorizeTransaction.getTransaction().setType(
				VelocityEnums.BankcardTransaction);
		authorizeTransaction.getTransaction().getTenderData().getCardData()
				.setCardType(req.getParameter("cardtype"));
		authorizeTransaction.getTransaction().getTenderData().getCardData()
				.setCardholderName(req.getParameter("cardHolderName"));
		authorizeTransaction.getTransaction().getTenderData().getCardData()
				.setPan(req.getParameter("panNumber"));
		authorizeTransaction
				.getTransaction()
				.getTenderData()
				.getCardData()
				.setExpire(req.getParameter("month") + req.getParameter("year"));
		// authorizeTransaction.getTransaction().getTenderData().getCardData().getTrack1Data().setNillable(true);
		authorizeTransaction.getTransaction().getTenderData().getCardData()
				.setTrack1Data1(req.getParameter("track1Data"));
		authorizeTransaction.getTransaction().getTenderData().getCardData()
				.setTrack2Data2(req.getParameter("track2Data"));
		authorizeTransaction.getTransaction().getTenderData()
				.getCardSecurityData().getAvsData().getCardholderName()
				.setNillable(true);
		authorizeTransaction.getTransaction().getTenderData()
				.getCardSecurityData().getAvsData()
		...
		authorizeTransaction.getTransaction().getTenderData()
				.getCardSecurityData().getAvsData()
				.setStateProvince(req.getParameter("state"));
		authorizeTransaction.getTransaction().getTenderData()
				.getCardSecurityData().getAvsData()
				.setPostalCode(req.getParameter("zip"));
		authorizeTransaction.getTransaction().getTenderData()
				.getCardSecurityData().getAvsData()
				.setPhone(req.getParameter("phone"));
		authorizeTransaction.getTransaction().getTenderData()
				.getCardSecurityData().getAvsData().getEmail()
				.setValue(req.getParameter("email"));
		authorizeTransaction.getTransaction().getTenderData()
				.getCardSecurityData().setCvDataProvided("Provided");
		authorizeTransaction.getTransaction().getTenderData()
				.getCardSecurityData().getCvData()
				.setValue(req.getParameter("cvc"));
		authorizeTransaction.getTransaction().getTenderData()
				.getCardSecurityData().getKeySerialNumber().setNillable(true);
		authorizeTransaction.getTransaction().getTenderData()
				.getCardSecurityData().getPin().setNillable(true);
		authorizeTransaction.getTransaction().getTenderData()
				.getCardSecurityData().getIdentificationInformation()
				.setNillable(true);
		authorizeTransaction.getTransaction().getTenderData()
				.getEcommerceSecurityData().setNillable(true);
		authorizeTransaction.getTransaction().getTransactionData()
				.setAmount(req.getParameter("amount"));
		authorizeTransaction.getTransaction().getTransactionData()
				.setCurrencyCode(req.getParameter("currencyCode"));
		authorizeTransaction
				.getTransaction()
				.getTransactionData()
				.setTransactiondateTime(req.getParameter("transactionDateTime"));
		authorizeTransaction.getTransaction().getTransactionData()
				.setAccountType(req.getParameter("accountType"));
		authorizeTransaction.getTransaction().getTransactionData()
				.setCustomerPresent(req.getParameter("customerpresent"));
		authorizeTransaction.getTransaction().getTransactionData()
				.setEmployeeId(req.getParameter("empID"));
		authorizeTransaction.getTransaction().getTransactionData()
				.setEntryMode(req.getParameter("entryMode"));
		authorizeTransaction.getTransaction().getTransactionData()
				.setIndustryType(req.getParameter("industryType"));
		authorizeTransaction.getTransaction().getTransactionData()
				.setInvoiceNumber("802");
		authorizeTransaction.getTransaction().getTransactionData()
				.setOrderNumber("629203");
		authorizeTransaction.getTransaction().getTransactionData()
				.setTipAmount(req.getParameter("tipAmount"));

		return authorizeTransaction;
	}

	/**
	 * Generating the Authorize input AuthorizeTransaction instance from the
	 * User inputs.
	 * 
	 * @author Vimal Kumar
	 * @param req
	 ...
	 * @return - of the type AuthorizeTransaction
	 */
	AuthorizeTransaction getAuthorizeRequestAuthorizeTransactionInstance(
			HttpServletRequest req) {
		...
		authorizeTransaction.getTransaction().setType(
				VelocityEnums.BankcardTransaction);
		/* Setting the values for Authorize XML */
		authorizeTransaction.getTransaction().getCustomerData()
				.getBillingData().getName().setNillable(true);
		authorizeTransaction.getTransaction().getCustomerData()
				.getBillingData().getAddress()
				.setStreet1(req.getParameter("street"));
		authorizeTransaction.getTransaction().getCustomerData()
				.getBillingData().getAddress().getStreet2().setNillable(true);
		authorizeTransaction.getTransaction().getCustomerData()
				.getBillingData().getAddress()
				.setCity(req.getParameter("city"));
		authorizeTransaction.getTransaction().getCustomerData()
		...
		authorizeTransaction.getTransaction().getCustomerData()
				...
				.setPostalCode(req.getParameter("zip"));
		authorizeTransaction.getTransaction().getCustomerData()
				.getBillingData().getAddress().setCountryCode("USA");
		authorizeTransaction.getTransaction().getCustomerData()
				.getBillingData()
				.setBusinessName(req.getParameter("businessName"));
		authorizeTransaction.getTransaction().getCustomerData()
				.getBillingData().getPhone().setNillable(true);
		authorizeTransaction.getTransaction().getCustomerData()
				.getBillingData().getFax().setNillable(true);
		authorizeTransaction.getTransaction().getCustomerData()
				.getBillingData().getEmail().setNillable(true);
		authorizeTransaction.getTransaction().getCustomerData()
				.setCustomerId(req.getParameter("empID"));
		authorizeTransaction.getTransaction().getCustomerData()
				.getCustomerTaxId().setNillable(true);
		authorizeTransaction.getTransaction().getCustomerData()
				.getShippingData().setNillable(true);
		authorizeTransaction.getTransaction().getReportingData()
				.setComment(req.getParameter("comment"));
		authorizeTransaction.getTransaction().getReportingData()
				.setDescription(req.getParameter("description"));
		authorizeTransaction.getTransaction().getReportingData()
				.setReference(req.getParameter("reference"));
		if (session.getAttribute("paymentAccount" + "") != null) {
			LOG.debug("getAuthorizeRequestAuthorizeTransactionInstance paymentAccountDataToken == "
					+ (String) session.getAttribute("paymentAccountDataToken"));
			authorizeTransaction
					.getTransaction()
					.getTenderData()
					.getPaymentAccountDataToken()
					.setValue(
							(String) session
									.getAttribute("paymentAccountDataToken"));
		} else {
			authorizeTransaction.getTransaction().getTenderData()
					.getPaymentAccountDataToken().setValue("");
		}
		authorizeTransaction.getTransaction().getTenderData()
				.getSecurePaymentAccountData().setNillable(true);
		authorizeTransaction.getTransaction().getTenderData()
				.getEncryptionKeyId().setNillable(true);
		authorizeTransaction.getTransaction().getTenderData().getSwipeStatus()
				.setNillable(true);
		authorizeTransaction.getTransaction().getTenderData().getCardData()
				.setCardType(req.getParameter("cardtype"));
		authorizeTransaction.getTransaction().getTenderData().getCardData()
		...
		authorizeTransaction
				...
				.setTransactionDateTime(req.getParameter("transactionDateTime"));
		authorizeTransaction.getTransaction().getTransactionData()
				.getCampaignId().setNillable(true);
		authorizeTransaction.getTransaction().getTransactionData()
				.setReference(req.getParameter("reference"));
		authorizeTransaction.getTransaction().getTransactionData()
				.setAccountType(req.getParameter("accountType"));
		authorizeTransaction.getTransaction().getTransactionData()
				.getApprovalCode().setNillable(true);
		authorizeTransaction.getTransaction().getTransactionData()
				.setCashBackAmount("0.0");
		authorizeTransaction.getTransaction().getTransactionData()
		...
		authorizeTransaction.getTransaction().getTransactionData()
				.setEntryMode(req.getParameter("entryMode"));
		authorizeTransaction.getTransaction().getTransactionData()
				.setGoodsType(req.getParameter("goodsType"));
		authorizeTransaction.getTransaction().getTransactionData()
				.setIndustryType(req.getParameter("industryType"));
		authorizeTransaction.getTransaction().getTransactionData()
				.getInternetTransactionData().setNillable(true);
		authorizeTransaction.getTransaction().getTransactionData()
				.setInvoiceNumber("");
		authorizeTransaction.getTransaction().getTransactionData()
				.setOrderNumber("629203");
		authorizeTransaction.getTransaction().getTransactionData()
				.setPartialShipment(false);
		authorizeTransaction.getTransaction().getTransactionData()
				.setSignatureCaptured(false);
		authorizeTransaction.getTransaction().getTransactionData()
				.setFeeAmount("1000.05");
		authorizeTransaction.getTransaction().getTransactionData()
				.getTerminalId().setNillable(true);
		authorizeTransaction.getTransaction().getTransactionData().getLaneId()
				.setNillable(true);
		authorizeTransaction.getTransaction().getTransactionData()
				.setTipAmount(req.getParameter("tipAmount"));
		authorizeTransaction.getTransaction().getTransactionData()
				.getBatchAssignment().setNillable(true);
		authorizeTransaction.getTransaction().getTransactionData()
				.setPartialApprovalCapable("NotSet");
		authorizeTransaction.getTransaction().getTransactionData()
				.getScoreThreshold().setNillable(true);
		authorizeTransaction.getTransaction().getTransactionData()
				.setQuasiCash(false);
		return authorizeTransaction;
	}
	...
	AuthorizeAndCaptureTransaction getAuthorizeAndCaptureTransactionInstance(
		...
		authorizeAndCaptureTransaction.getTransaction().setType(
				VelocityEnums.BankcardTransaction);
		authorizeAndCaptureTransaction.getTransaction().getCustomerData()
				.getBillingData().getName().setNillable(true);
		authorizeAndCaptureTransaction.getTransaction().getCustomerData()
				.getBillingData().getAddress()
				.setStreet1(req.getParameter("street"));
		...
		authorizeAndCaptureTransaction.getTransaction().getCustomerData()
				.getBillingData().getAddress()
				.setCountryCode(req.getParameter("country"));
		authorizeAndCaptureTransaction.getTransaction().getCustomerData()
				.getBillingData()
				.setBusinessName(req.getParameter("businessName"));
		authorizeAndCaptureTransaction.getTransaction().getCustomerData()
		...
		authorizeAndCaptureTransaction.getTransaction().getCustomerData()
				.getShippingData().setNillable(true);
		authorizeAndCaptureTransaction.getTransaction().getReportingData()
				.setComment(req.getParameter("comment"));
		authorizeAndCaptureTransaction.getTransaction().getReportingData()
				.setDescription(req.getParameter("description"));
		authorizeAndCaptureTransaction.getTransaction().getReportingData()
		...
		if (session.getAttribute("paymentAccountDataToken") != null) {
			LOG.debug("getAuthorizeAndCaptureTransactionInstance paymentAccountDataToken == "
					+ (String) session.getAttribute("paymentAccountDataToken"));
		...
		authorizeAndCaptureTransaction.getTransaction().getTenderData()
				.getSecurePaymentAccountData().setNillable(true);
		authorizeAndCaptureTransaction.getTransaction().getTenderData()
				.getEncryptionKeyId().setNillable(true);
		authorizeAndCaptureTransaction.getTransaction().getTenderData()
				.getSwipeStatus().setNillable(true);
		authorizeAndCaptureTransaction.getTransaction().getTenderData()
				.getCardData().setCardType(req.getParameter("cardtype"));
		authorizeAndCaptureTransaction.getTransaction().getTenderData()
				.getCardData().setPan(req.getParameter("panNumber"));
		authorizeAndCaptureTransaction
				.getTransaction()
				.getTenderData()
				.getCardData()
				.setExpire(req.getParameter("month") + req.getParameter("year"));
		authorizeAndCaptureTransaction.getTransaction().getTenderData()
				.getCardData().setTrack1Data1(req.getParameter("track1Data"));
		authorizeAndCaptureTransaction.getTransaction().getTenderData()
				.getCardData().setTrack2Data2(req.getParameter("track2Data"));
		authorizeAndCaptureTransaction.getTransaction().getTenderData()
				.getEcommerceSecurityData().setNillable(true);
		authorizeAndCaptureTransaction.getTransaction().getTransactionData()
				.setAmount(req.getParameter("amount"));
		authorizeAndCaptureTransaction.getTransaction().getTransactionData()
				.setCurrencyCode(req.getParameter("currencyCode"));
		authorizeAndCaptureTransaction
	...
	ChangeTransaction getCaptureTransactionInstance(HttpServletRequest req) {
		...
		captureTransaction.getDifferenceData().setType(
				VelocityEnums.BankcardCapture);
		captureTransaction.getDifferenceData().setAmount(
				req.getParameter("amount"));
		captureTransaction.getDifferenceData().setTipAmount(
				req.getParameter("tipAmount"));
	...
	Adjust getAdjustTransactionInstance(HttpServletRequest req) {
		...
		adjustTransaction.getDifferenceData().setAmount(
				req.getParameter("amount"));
	...
	ReturnById getReturnByIdTransactionInstance(HttpServletRequest req) {
		...
		returnByIdTransaction.getDifferenceData().setAmount(
				req.getParameter("amount"));
	...
	ReturnTransaction getReturnTransactionInstance(HttpServletRequest req) {
		...
		returnUnlinkedTransaction.getTransaction().setType(
				VelocityEnums.BankcardTransaction);
		returnUnlinkedTransaction.getBatchIds().setNillable(true);
		returnUnlinkedTransaction.getTransaction().getCustomerData()
				.getBillingData().getName().setNillable(true);
		returnUnlinkedTransaction.getTransaction().getCustomerData()
				.getBillingData().getAddress()
				.setStreet1(req.getParameter("street"));
		...
		returnUnlinkedTransaction.getTransaction().getCustomerData()
				.getShippingData().setNillable(true);
		returnUnlinkedTransaction.getTransaction().getReportingData()
				.setComment("a test comment");
		returnUnlinkedTransaction.getTransaction().getReportingData()
				.setDescription(req.getParameter("description"));
		returnUnlinkedTransaction.getTransaction().getReportingData()
				.setReference(req.getParameter("reference"));
		if (session.getAttribute("paymentAccountDataToken" + "") != null) {
			LOG.debug("getReturnTransactionInstance paymentAccountDataToken == "
					+ (String) session.getAttribute("paymentAccountDataToken"));
			returnUnlinkedTransaction
					.getTransaction()
					.getTenderData()
					.getPaymentAccountDataToken()
					.setValue(
		...
		returnUnlinkedTransaction.getTransaction().getTenderData()
				.getSecurePaymentAccountData().setNillable(true);
		returnUnlinkedTransaction.getTransaction().getTenderData()
				.getEncryptionKeyId().setNillable(true);
		returnUnlinkedTransaction.getTransaction().getTenderData()
		...
		returnUnlinkedTransaction.getTransaction().getTenderData()
				.getEcommerceSecurityData().setNillable(true);
		returnUnlinkedTransaction.getTransaction().getTransactionData()
				.setAmount(req.getParameter("amount"));
		returnUnlinkedTransaction.getTransaction().getTransactionData()
				.setCurrencyCode(req.getParameter("currencyCode"));
		returnUnlinkedTransaction
	 ...
	 */
	QueryTransactionsDetail getQueryTransactionsDetailInstance(
			HttpServletRequest req) {
		HttpSession session = req.getSession(false);
		...
		queryTransactionsDetail
				.setTransactionDetailFormat(com.velocity.enums.TransactionDetailFormat.CWSTransaction);
	...
	AuthorizeTransaction getP2PEAuthorizeRequestAuthorizeTransactionInstance(
			HttpServletRequest req) {
		AuthorizeTransaction authorizeTransaction = new AuthorizeTransaction();
		// Setting the values for Authorize XML
		authorizeTransaction.getTransaction().setType(
		...
		authorizeTransaction.getTransaction().getCustomerData()
				.getBillingData().getAddress()
				.setStreet1(req.getParameter("street1"));
		authorizeTransaction.getTransaction().getCustomerData()
				.getBillingData().getAddress()
				.setCity(req.getParameter("city"));
		authorizeTransaction.getTransaction().getCustomerData()
				.getBillingData().getAddress()
				.setStateProvince(req.getParameter("stateProvince"));
		authorizeTransaction.getTransaction().getCustomerData()
				.getBillingData().getAddress()
				.setPostalCode(req.getParameter("zip"));
		authorizeTransaction.getTransaction().getCustomerData()
		...
		authorizeTransaction.getTransaction().getReportingData()
				.setReference(req.getParameter("reference"));
		authorizeTransaction.getTransaction().getTenderData()
				.getPaymentAccountDataToken().setNillable(true);
		authorizeTransaction.getTransaction().getTenderData()
				.getSecurePaymentAccountData()
				.setValue(req.getParameter("securePaymentAccountData"));
		authorizeTransaction.getTransaction().getTenderData()
				.getEncryptionKeyId()
				.setValue(req.getParameter("encryptionKeyId"));
		authorizeTransaction.getTransaction().getTenderData().getSwipeStatus()
				.setValue(req.getParameter("swipeStatus"));
		authorizeTransaction.getTransaction().getTenderData()
				.getCardSecurityData().getIdentificationInformation()
				.setValue((req.getParameter("identificationInformation")));
		authorizeTransaction.getTransaction().getTransactionData()
				.setAmount(req.getParameter("amount"));
		authorizeTransaction.getTransaction().getTransactionData()
				.setCurrencyCode(req.getParameter("currencyCode"));
	 ...
	 * 
	 * @author Vimal Kumar The method sets the values for CaptureAll XML
	 *         elements.
	...
	AuthorizeAndCaptureTransaction getP2PEAuthorizeAndCaptureRequestAuthorizeTransactionInstance(
			HttpServletRequest req) {
		AuthorizeAndCaptureTransaction authorizeAndCaptureTransaction = new AuthorizeAndCaptureTransaction();
		// Setting the values for AuthorizeAndCapture XML
		authorizeAndCaptureTransaction.getTransaction().setType(
		...
		authorizeAndCaptureTransaction.getTransaction().getTenderData()
				...
				.setValue(req.getParameter("encryptionKeyId"));
		authorizeAndCaptureTransaction.getTransaction().getTenderData()
				.getSwipeStatus().setValue(req.getParameter("swipeStatus"));
		authorizeAndCaptureTransaction.getTransaction().getTenderData()
				.getCardSecurityData().getIdentificationInformation()
				.setValue((req.getParameter("identificationInformation")));
		authorizeAndCaptureTransaction.getTransaction().getTransactionData()
